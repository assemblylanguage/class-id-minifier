"use strict";
/**
 * Code for minifying all of the class names and IDs in the document.
 */
Object.defineProperty(exports, "__esModule", { value: true });
exports.minifyClassNamesAndIDsToAttributes = exports.minifyIDs = exports.minifyClassNames = exports.createMinifiedNameFromNumber = void 0;
 ...
 * the case.
 */
function createMinifiedNameFromNumber(base10Number) {
    ...
    const base26CharArray = base26Array.map((base26Number) => {
        return String.fromCharCode(base26Number + ASCII_OFFSET);
    });
    // Joining the characters together into a base-26 numeric string and
    // returning it.
    const base26NumericString = base26CharArray.join('');
    return base26NumericString;
}
exports.createMinifiedNameFromNumber = createMinifiedNameFromNumber;
/**
    ...
    for (const element of classNameElements) {
        ...
        for (const className of classList) {
            ...
            if (!(className in minifiedClassMap)) {
                ...
                const minifiedClassName = createMinifiedNameFromNumber(minifiedClassCount);
                ...
                const minifiedClassName = minifiedClassMap[className];
                element.classList.replace(className, minifiedClassName);
            }
        }
    }
...
exports.minifyClassNames = minifyClassNames;
/**
...
function minifyIDs(document) {
    ...
    for (const element of idElements) {
        ...
        if (!(id in minifiedIdMap)) {
            ...
            const minifiedID = createMinifiedNameFromNumber(minifiedIdCount);
            minifiedIdCount += 1;
    ...
    for (const element of inlineHrefElements) {
        ...
        // minified ID value.
        const minifiedId = minifiedIdMap[hash];
        element.setAttribute('href', `#${minifiedId}`);
    }
}
exports.minifyIDs = minifyIDs;
/**
 ...
 * @param document the document that will be modified in place.
 */
function minifyClassNamesAndIDsToAttributes(document) {
    ...
    // attributes.
    let minifiedAttributeCount = 0;
    const minifiedClassToAttributeMap = {};
    const minifiedIdToAttributeMap = {};
    // Getting all of the class names within the document, and populating the
    ...
    for (const element of classNameElements) {
        ...
        for (const className of classList) {
            ...
            if (!(className in minifiedClassToAttributeMap)) {
                ...
                const minifiedHtmlAttribute = createMinifiedNameFromNumber(minifiedAttributeCount);
                ...
                const minifiedCssAttribute = minifiedClassToAttributeMap[className];
    ...
    for (const element of inlineHrefElements) {
        ...
        // value.
        const minifiedId = minifiedIdToAttributeMap[hash];
    ...
    for (const styleElement of styleElements) {
        ...
        cssTree.walk(cssAst, (node) => {
            ...
            if (node.type === 'IdSelector'
                ...
                const minifiedId = minifiedIdToAttributeMap[node.name];
    ...
    for (const element of idElements) {
        ...
        if (Object.values(minifiedIdToAttributeMap).includes(id)
...
exports.minifyClassNamesAndIDsToAttributes = minifyClassNamesAndIDsToAttributes;
